...
function childEnv() {
  ...
  // Cursor Work isola HOME; reusa gh/npm do host macOS quando existirem.
  const macHome = "/Users/pauloborini";
  if (!env.GH_CONFIG_DIR && !env.GH_TOKEN && !env.GITHUB_TOKEN) {
    const macGh = join(macHome, ".config/gh");
    if (existsSync(join(macGh, "hosts.yml"))) {
      env.GH_CONFIG_DIR = macGh;
    }
  }
  if (!env.NODE_AUTH_TOKEN && !env.NPM_CONFIG_USERCONFIG) {
    const macNpmrc = join(macHome, ".npmrc");
