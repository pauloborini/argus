...
## 11. Consumo Talos HANDOFF → `remember` / `recall` (S06)

Contrato de **consumo** do path Argus do adapter Talos (`$talos-memory-promote`). Não reabre o ciclo memória v2 Argus S01–S08; não muda o wire das seções 1–10.

**Origem:** artefato descartável `HANDOFF_*.md` sob `.talos/memory/` (template Talos S02), promovido pela skill `$talos-memory-promote` / `sink_adapter.rememberCallShape`.

### Mapping HANDOFF → args nativos `remember`

| Campo HANDOFF / candidato | Argus `remember` |
|---------------------------|------------------|
| `claim` (+ `motivo` se houver) | `content` (motivo anexado após claim com ` — `) |
| (fixo) | `type: "decision"` |
| origem Talos | tag `talos-handoff` |
| `âncora.tipo` + `âncora.valor` | tag `anchor:<tipo>:<valor>` (omitir se âncora ausente) |
| `ref` presente / truthy | `links: [ref]` |
| `ref` ausente / null | omitir `links` (ou `[]`) |

Tags estáveis mínimas: sempre `talos-handoff`; âncora forte vira `anchor:<tipo>:<valor>` quando presente.

### Ref 1+1 (não bloqueia promote)

Quando `ref` aponta path inexistente: tentar **1** leitura do path → **1** name search pelo basename → se falhar, **seguir só com o fato** (promote/remember sem travar). Se a ausência da ref já for conhecida, pular lookup.

### Retrieval pós-promote

Preferir `recall` (e/ou `pack_context` sob budget) para achar claim/âncora. Proibido dump do vault. Prova formal de smoke (Talos **S08**) vive em `packages/argus/tests/memory/promote-recall-smoke.test.ts` (remember shape `talos-handoff` → recall same-session; omit/broken `links` sem hard-fail); regressão dream seguro permanece em `daemon-dream-schedule.test.ts` + `dream-cycle.test.ts` (S07).

### Soft-fail sem Argus

Se MCP `remember` estiver ausente, o promote Talos faz **soft-fail** (S04 / D14): o `HANDOFF_*.md` permanece no disco; Argus continua **opcional** no pipeline Talos. S06 não inverte esse comportamento.

### Won't (proibido neste contrato)

- Criar API / CLI `argus learn`
- Receipt JSON ou endpoint novo de ingestão além de `remember` / `recall` existentes
- Tornar Argus obrigatório no pipeline Talos
