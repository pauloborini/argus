...
export async function runInstall(options: InstallOptions = {}): Promise<number> {
  ...
  // 3. Agent-rules (alavanca portável para o agente usar o Cortex)
  ...
  if (!options.noDaemon) {
    ...
    if (isDaemonRunning()) {
      ...
      if (startCode === 0 && (await waitForDaemon())) {
        ...
        hadPartialFailure = true;
        summary.push(
          "daemon NÃO subiu — verifique 'cortex daemon status' e rode 'cortex daemon start'",
        );
      }
    }
  ...
  console.log("\n✓ Cortex instalado neste repositório:");
...
export function runUninstall(options: UninstallOptions = {}): number {
  ...
  if (options.purge) {
    ...
    console.log("  .cortex/ removido (--purge)");
  }

  console.log("\n✓ Cortex desinstalado deste repositório.");
  return 0;
}
