...
// lexical BM25. Off-by-default — depende de `cortex embed` ter populado os
  ...
  try {
    ...
    return {
      ...
      ...stubResponse("parcial", EMBEDDINGS_UNAVAILABLE, {
        limitations: ["Busca semântica indisponível; fallback lexical. Execute cortex embed."],
  ...
  try {
    ...
    return {
      ...
      ...stubResponse(
        ...
        stale ? { limitations: ["Embeddings defasados; refresque com cortex embed."], staleness_hint: EMBEDDINGS_HINT } : undefined,
      ),
    };
  } finally {
    closeIndexDb(db);
