  ...
  try {
    ...
    if (!isManifestLike(parsed)) {
      throw new ManifestCorruptedError(
        "E_INDEX_CORRUPTED: Manifest corrompido ou inválido; execute argus index para recriar o inventário.",
      );
    }
    return parsed;
  } catch (err) {
    ...
    throw new ManifestCorruptedError(
      "E_INDEX_CORRUPTED: Manifest ilegível; execute argus index para recriar o inventário.",
    );
  }
}
