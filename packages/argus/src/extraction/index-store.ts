  ...
  try {
    ...
    if (!isStructuralIndexLike(parsed)) {
      throw new StructuralIndexCorruptedError(
        "E_INDEX_CORRUPTED: Índice estrutural corrompido ou inválido; execute argus index para reconstruir.",
      );
    }
    return parsed;
  } catch (err) {
    ...
    throw new StructuralIndexCorruptedError(
      "E_INDEX_CORRUPTED: Índice estrutural ilegível; execute argus index para reconstruir.",
    );
  }
}
