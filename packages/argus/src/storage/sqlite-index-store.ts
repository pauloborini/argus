...
export function openIndexDb(dbPath: string, options?: { readonly?: boolean }): Database {
  ...
  if (!options?.readonly) {
    ...
    throw new IndexDbCorruptedError(
      "E_INDEX_MISSING: Índice SQLite ausente; execute argus index para gerar .argus/index.db.",
    );
  }
  ...
  try {
    ...
    throw new IndexDbCorruptedError(
      "E_INDEX_CORRUPTED: Banco SQLite ilegível ou corrompido; execute argus index para reconstruir.",
    );
  }
...
export function readIndexMeta(db: Database): IndexMeta | null {
  ...
  try {
    ...
    throw new IndexDbCorruptedError(
      "E_INDEX_CORRUPTED: Metadados do índice SQLite ilegíveis; execute argus index para reconstruir.",
    );
  }
}
...
export function readAllFileEntries(db: Database): FileStructuralEntry[] {
  ...
  return files.map((file) => {
    ...
    try {
      ...
      throw new IndexDbCorruptedError(
        "E_INDEX_CORRUPTED: Dados de arquivo corrompidos no SQLite; execute argus index para reconstruir.",
      );
    }
