...
// por padrão — `embeddings` fica vazia até `argus embed` rodar, então DBs
...
export function applyMigrations(db: Database): void {
  ...
  if (appliedVersion > MIGRATION_VERSION) {
    throw new IndexDbSchemaError(
      `E_INDEX_SCHEMA_INCOMPATIBLE: Schema do banco (v${appliedVersion}) é mais novo que o runtime; atualize o pacote argus.`,
    );
  }
...
export function assertCompatibleIndexSchema(db: Database): void {
  ...
  if (meta.schema_version !== SQLITE_SCHEMA_VERSION) {
    throw new IndexDbSchemaError(
      `E_INDEX_SCHEMA_INCOMPATIBLE: Schema do índice (${meta.schema_version}) incompatível com runtime (${SQLITE_SCHEMA_VERSION}); execute argus index para reconstruir.`,
    );
  }
}
