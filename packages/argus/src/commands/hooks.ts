...
function buildBlock(baseExpr: string, hookName: string, rootPath: string): string {
  ...
  const guard =
    hookName === "post-checkout"
      ? '[ "$3" = "1" ] || exit 0\n'
      : "";
  // Roda no root do WORKSPACE argus (não no git toplevel): em monorepo o
  // `.argus` pode estar num subdiretório, e `argus` resolve o workspace a
  ...
  return [
    BLOCK_BEGIN,
    "# Gerado por `argus hook install`. Não edite à mão.",
    `cd ${root} || exit 0`,
    guard +
...
export function runHookInstall(cwd: string = process.cwd()): number {
  ...
  if (!hooksDir) {
    console.error(
      "E_NO_GIT: Repositório git não encontrado. Hooks exigem git; use `argus sync` manual ou auto-sync do MCP.",
    );
    return 1;
  }
...
export function runHookUninstall(cwd: string = process.cwd()): number {
  ...
  console.log(`Hooks argus removidos: ${removed}. Hooks pré-existentes do usuário preservados.`);
  return 0;
}
