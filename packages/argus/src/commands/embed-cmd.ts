 ...
 * roda só quando o usuário invoca `argus embed` (nem `index` nem o daemon
...
export async function runEmbed(options: EmbedOptions = {}): Promise<number> {
  ...
  if (!existsSync(dbPath)) {
    console.error("E_INDEX_MISSING: Índice ausente; execute argus index antes de argus embed.");
    return 1;
  }
  ...
  try {
    if (!isIndexDbPopulated(db)) {
      console.error("E_INDEX_MISSING: Índice vazio; execute argus index antes de argus embed.");
      return 1;
    }
