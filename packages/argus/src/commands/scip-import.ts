 ...
 * Off-by-default: roda apenas via `argus scip import`, não auto-sincroniza.
 */
export async function runScipImport(options: ScipImportOptions = {}): Promise<number> {
  ...
  if (!existsSync(dbPath)) {
    console.error("E_INDEX_MISSING: Índice ausente; execute argus index antes de argus scip import.");
    return 1;
  }
  ...
  try {
    if (!isIndexDbPopulated(db)) {
      console.error("E_INDEX_MISSING: Índice vazio; execute argus index antes de argus scip import.");
      return 1;
    }
