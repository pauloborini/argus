...
program
  .name("argus")
  .description("Argus — CLI local de retrieval e context packing")
...
program
  .command("init")
  .description("Preparar workspace e metadados locais em .argus/ (use 'install' para fiação completa)")
  .action(() => {
    finish(runInit());
  });
...
program
  ...
  .option("--purge", "Remover também o diretório .argus/ (índice local)")
...
program
  ...
  .option("--mcp", "Iniciar servidor MCP stdio (argus)")
...
const daemon = program
  ...
  .action(async () => {
    // `argus daemon` sem subcomando roda em foreground (alvo do serviço).
...
hook
  .command("uninstall")
  .description("Remover hooks git do argus (preserva hooks do usuário)")
  .action(() => {
...
agentRules
  .command("install")
  .description("Escrever bloco de regras argus em CLAUDE.md e AGENTS.md")
  .action(() => {
    finish(runAgentRulesInstall());
  });
agentRules
  .command("uninstall")
  .description("Remover bloco de regras argus de CLAUDE.md e AGENTS.md")
  .action(() => {
